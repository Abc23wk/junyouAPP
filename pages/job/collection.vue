	...
	<view class="content">
		<view v-if="list.length > 0">
			<van-cell-group custom-class="lxk-zw" :border="false" v-for="(item, index) in list" :key="index">
				<van-cell :title="item.job.title" use-label-slot :border="false" @click="detailTo(item.job.id)">
					<view class="lxk-price">
						...
						<view class="time">
							<wux-timeago :to="item.job.pubdate * 1000" lang="zh_CN" />
						</view>
					</view>
					<view slot="label">
		...
		<view class="lxk-add" v-else-if="ready">
			...
			<view class="lxk-add-title">暂未收藏任何职位</view>
			<view class="lxk-add-desc">去搜索并收藏职位，以获取最新信息</view>
	...
	export default {
		...
		methods: {
			getJobCollection() {
				...
				getJobCollection(search).then(response => {
					let r = response.data
